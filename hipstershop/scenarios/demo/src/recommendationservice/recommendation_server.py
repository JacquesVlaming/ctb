...
def simulate_issue():
    factorial(20000) / 0  # CPU saturation, latency, and error
...
class RecommendationService(demo_pb2_grpc.RecommendationServiceServicer):
    ...
    def ListRecommendations(self, request, context):
        ...
        with elasticapm.capture_span(
            ...
            try:
                logger.warn('simulate_issue()', extra=get_extra_logging_payload())
                simulate_issue()
            except ZeroDivisionError as e:
                client.capture_exception()
                elasticapm.set_custom_context({'request': request_dict })
                client.end_transaction('/hipstershop.RecommendationService/ListRecommendations', 'failure')
                raise e
...
if __name__ == '__main__':
    ...
    # start server
    logger.info('listening on 0.0.0.0:' + port, extra= get_extra_logging_payload())
